...
const char col_str[7][10] = {"yellow ", 
                             "white  ", 
                             "orange ", 
                             "red    ", 
                             "blue   ", 
...
// Globale Variablen
int cube[6][9];             // Speichermodell des Würfels mit allen Farben


//-----------------------------------------------------------------
// Setzt die Farben nach dem Einschalten des Systems
void init_cube_colors(){
  ...
  cube[0][8] = 5;   // green (left)
  cube[1][8] = 2;   // orange (front)
  cube[2][8] = 4;   // blue (right)
  cube[3][8] = 3;   // red (back)
  cube[4][8] = 0;   // yellow (top)
  cube[5][8] = 1;   // white (bottom)
}

//-----------------------------------------------------------------
// Prints the colors from all sides of the cube
void print_cube_colors(void) {
  Serial.println("Cube");
  for(int i = 0; i < 6; ++i) {
    Serial.print("Seite "); Serial.print(i); Serial.print(": ");
    for(int j = 0; j < 9; ++j) {
      if (cube[i][j] >= 0) Serial.print(col_str[i]);
      else Serial.print("?      ");
    }
    Serial.println();
  }
...
int decode_col_edge(void) {
  ...
  const int col_tab[6][8] = {
    ...
    165, 205,    250, 315,     305,  390,    36, 160,   // blue
    200, 285,    350, 540,     250,  330,    45, 100,   // green
  };
